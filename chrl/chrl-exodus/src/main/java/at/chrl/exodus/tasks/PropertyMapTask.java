...
public final class PropertyMapTask extends Task {
	...
	public void runTask() {
		...
		final THashSet<String> outputData = new THashSet<>(300);// <String,
																// String>(300);
		...
		// FileInputStream(fileToRead), Charset.forName("UTF-8")), 8192<<4);
		...
		try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(fileToRead), Charset.forName("UTF-8")), 8192 << 4)) {
			br.lines().distinct().forEach(strLine -> perLine(outputData, strLine));
