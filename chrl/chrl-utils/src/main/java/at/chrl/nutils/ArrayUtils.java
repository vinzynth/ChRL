/**
 * (C) ChRL 2014 - chrl-utils - at.chrl.nutils - ArrayUtils.java
 * Created: 02.08.2014 - 20:33:52
 */
...
public class ArrayUtils {
    ...
    public static <T> Function<T[], ?> getToPrimitive(Class<T> clazz){
    	if(clazz.equals(Integer.class) 		|| clazz.equals(Integer.TYPE))		return s -> toPrimitive((Integer[])s);
    	if(clazz.equals(Byte.class)			|| clazz.equals(Byte.TYPE))			return s -> toPrimitive((Byte[])s);
    	if(clazz.equals(Character.class)	|| clazz.equals(Character.TYPE))	return s -> toPrimitive((Character[])s);
    	if(clazz.equals(Double.class)		|| clazz.equals(Double.TYPE))		return s -> toPrimitive((Double[])s);
    	if(clazz.equals(Long.class)			|| clazz.equals(Long.TYPE))			return s -> toPrimitive((Long[])s);
    	if(clazz.equals(Float.class)		|| clazz.equals(Float.TYPE))		return s -> toPrimitive((Float[])s);
    	if(clazz.equals(Boolean.class)		|| clazz.equals(Boolean.TYPE))		return s -> toPrimitive((Boolean[])s);
    	if(clazz.equals(Short.class)		|| clazz.equals(Short.TYPE))		return s -> toPrimitive((Short[])s);
    	return ArrayUtils::nop;
    }
    
    private static <T> Object nop(T[] array){
    	return array;
    }
